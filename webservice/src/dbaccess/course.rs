...
pub async fn post_new_course_db(pool:&PgPool,new_course:CreateCourse)->Result<Course,MyError>{
    let row=sqlx::query_as!(Course,
        ...
        new_course.teacher_id,new_course.name,new_course.description,new_course.format,new_course.structure,new_course.duration,
...
pub async fn update_course_details_db(pool:&PgPool,teacher_id:i32,id:i32,update_course:UpdateCourse)->Result<Course,MyError>{
    ...
    let current_course_row=sqlx::query_as!(Course,"select * from course where teacher_id=$1 and id=$2",teacher_id,id).fetch_one(pool).await
    ...
    let course_row=sqlx::query_as!(Course,
        ...
        where teacher_id=$9 and id=$10 returning id,teacher_id,name,description,format,structure,duration,price,language,level,time",
